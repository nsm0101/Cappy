...
**Goal:** Get the `/app` React Native app to a functioning, device-testable alpha wired to the live `cappy-dev` Supabase backend.
...
| App root | `app/` (Expo SDK 51, RN 0.74, TS strict) |
...
**Status:** DONE (2026-06-30) — `cappy-mark.png` 1.21 MB → **52 KB** (resized 1250→600 px + 256-color quantize), `cappy-icon.png` 279 KB → **36 KB** (256-color quantize). Both well under target; flat-logo quality preserved.
...
**Status:** DONE (2026-06-30) — HomeScreen now derives each child's dose status from the **most-recently-dosed medication** via the `compute_dose_status` RPC (real interval/caps), replacing the 30-minute time heuristic. No schema change needed — the last dose's `medication_id` is the signal.
...
- **TEST-1** smoke tests · **CI-1** GitHub Actions · **LEG-1** privacy/terms/consent (counsel) · **DOC-1** docs.
- ✅ **PERF-1** image optimization — DONE (mark 52 KB, icon 36 KB). ✅ **FEAT-2** real dose status on Home — DONE (compute_dose_status RPC).
- **FEAT-1** manual "adjust amount" — largely obsoleted by the computed recommended dose.
